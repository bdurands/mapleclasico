...
#include <string>
#include "theactualstuff/BossHP.h"
#include "Discord/discord.h"
#include "HackStuff/loginstuff.h"
#include "HackStuff/haxAddresses.h"
#include "API/MapleStory/Type/ZArray.h"
...
void doini()
{
	...
	if (reader.ParseError() == 0) {
		...
		if (reader.GetBoolean("discord", "enableactivitystatus", "true"))
		{
			DiscordRichPresence::getInstance().beginUpdates();
		}
		...
		if (reader.GetBoolean("clienthax", "jumpshoot", "true")) {
			// CUserLocal::TryDoingShootAttack
			WriteByte(0x009539FA, 0xE9);
			WriteValue(0x009539FA + 1, 0x00953A11 - 0x009539FA - 5);
		}

		if (reader.GetBoolean("clienthax", "jumpmagic", "true")) {
			// CUserLocal::TryDoingShootAttack
			WriteByte(0x009559E5, 0xE9);
			WriteValue(0x009559E5 + 1, 0x00955A20 - 0x009559E5 - 5);
		}

		Hook_CWvsApp__EnableWinkey(reader.GetBoolean("clienthax", "enablewinkey", "true"));
	}
}
...
const char* __cdecl get_job_name(int nJob)
{
	...
	case 2113:
		...
		//
	case 213:
		return "Fire Grand Master";
	case 223:
		return "Ice Grand Master";
	case 233:
		return "Holy Grand Master";
	case 1213:
		return "Fiercing Flame";
...
const char* coolTime2 = "UI/UIWindow.img/Skill/CoolTime2/%d";

// Original return address and function
const DWORD skillCoolTimePathRtn = 0x007B455C;
const DWORD funcSkillCoolTimePath = 0x00406455;

// Hook to replace the skill cooldown path (changes the image path used)
__declspec(naked) void SkillCoolTimePath_Hook()
{
	__asm {
		call funcSkillCoolTimePath
		mov eax, [coolTime2]
		jmp skillCoolTimePathRtn
	}
}

// Custom function to adjust display time of cooldown indicator
unsigned int __cdecl RemainTime(unsigned int leftTime)
{
	unsigned int seconds = leftTime / 1000;

	if (seconds > 60)
	{
		const unsigned int minutes = seconds / 60;
		const unsigned int result = minutes + 60;
...
// Hook to replace the shadow index calculation
__declspec(naked) void UpdateShadowIndex_Hook()
{
	__asm {
		mov eax, [edi + 0x38]
		push eax
...
// Apply the hooks and memory patches
void ApplyCooldownUIFix()
{
	CodeCave(SkillCoolTimePath_Hook, 0x007B4555, 7);
	CodeCave(UpdateShadowIndex_Hook, 0x007B451B, 6);

	// Set the maximum value for the shadow index bar
	WriteByte(0x007B4521 + 2, 120);
	WriteByte(0x007B4526 + 1, 120);
}
//////////////////////////////////////////////////////////////////////////////////

/////NEW EQUIPMENT SLOTS WIP///////////

// ===== Shoulder Equipment Slot Extension (Hujiankuozhan) =====
...
// Equipment grid mapping
// Equipment Grid Layout (7 rows x 5 columns)
int equipGridMap[7][5] = {
	/* Row 1 */ { UNKNOW,      HAT,          UNKNOW,      UNKNOW,       UNKNOW      },
	/* Row 2 */ { MEDAL,       FACE_ACCESSORY,     UNKNOW,      RING_3,      RING_4 },
	/* Row 3 */ { UNKNOW,	   UNKNOW,       EYE,         EAR,          SHOULDER    },
	/* Row 4 */ { CAPE,        TOP,          PENDANT,     WEAPON,       SHIELD      },
	/* Row 5 */ { GLOVES,      PANTS,        BELT,        RING_1,       RING_2      },
	/* Row 6 */ { UNKNOW,      UNKNOW,       SHOES,       RING_5,       RING_6      },
	/* Row 7 */ { TAMED_MOB,   SADDLE,       TAMINGMOB_EQUIP, UNKNOW,   UNKNOW      }
};


int equipGridMapX[5] = { 5, 38, 71, 104, 137 };
int equipGridMapY[7] = { 35, 68, 100, 132, 166, 197, 233 };

// Hook function pointers
typedef void* (__cdecl* GetItemCategoryNameType)(DWORD* a1, signed int nItemId);
...
typedef bool(__cdecl* IsCorrectBodyPartType)(int nItemID, int nBodyPart, int nGender);

GetItemCategoryNameType OriginalGetItemCategoryName = (GetItemCategoryNameType)0x005C9E61;
CUIEquipGetBodyPartFromPointType OriginalCUIEquipGetBodyPartFromPoint = (CUIEquipGetBodyPartFromPointType)0x007FEC32;
GetBodyPartFromItemType OriginalGetBodyPartFromItem = (GetBodyPartFromItemType)0x004606A0;
IsCorrectBodyPartType OriginalIsCorrectBodyPart = (IsCorrectBodyPartType)0x00460358;

DWORD dwPutEquipPositionHookRet = 0x007FEF58;

// Forward declaration for hook
void putEquipPosition_Hook();

// ===== Equipment Grid Functions =====
int getEquipGridPosition(int cursorX, int cursorY) {
	for (int y = 0; y < 7; y++) {
		for (int x = 0; x < 5; x++) {
			int topLeftX = equipGridMapX[x];
			int topLeftY = equipGridMapY[y];
			int bottomRightX = topLeftX + 32;
			int bottomRightY = topLeftY + 32;
			if (cursorX >= topLeftX && cursorX < bottomRightX && cursorY >= topLeftY && cursorY < bottomRightY) {
...
int getPixelPositionXFromNumber(int number) {
	for (int y = 0; y < 7; ++y)
		for (int x = 0; x < 5; ++x)
			if (equipGridMap[y][x] == number) return equipGridMapX[x];
	return -1;
}

int getPixelPositionYFromNumber(int number) {
	for (int y = 0; y < 7; ++y)
		for (int x = 0; x < 5; ++x)
			if (equipGridMap[y][x] == number) return equipGridMapY[y] + 32;
	return -1;
}

// ===== Hook Definitions =====
__declspec(naked) void putEquipPosition_Hook() {
	__asm {
		mov eax, [ebp + 0x8]
		push eax
		...
		pop eax
		push edi
		jmp dwPutEquipPositionHookRet
	}
}

void* __cdecl GetItemCategoryName_Hook(DWORD* result, int nItemId) {
	auto ret = OriginalGetItemCategoryName(result, nItemId);
	if (nItemId / 10000 == 115) {
		// Optionally set "Shoulder" category label here if needed
	}
	return ret;
}
...
int __fastcall CUIEquip_GetBodyPartFromPoint_Hook(void* pThis, void* edx, int rx, int ry) {
	return getEquipGridPosition(rx, ry);
}
...
int __cdecl GetBodyPartFromItem_Hook(int nItemID, int nGender, int* pnBodyPart, int bAll) {
	...
	return OriginalGetBodyPartFromItem(nItemID, nGender, pnBodyPart, bAll);
}


bool __cdecl IsCorrectBodyPart_Hook(int nItemID, int nBodyPart, int nGender) {
	bool ret = OriginalIsCorrectBodyPart(nItemID, nBodyPart, nGender);
	if (nItemID / 10000 == 115) ret = (nBodyPart == SHOULDER);
	return ret;
}


// ===== Memory Patches =====
void ApplyShoulderPatches() {
	WriteByte(0x005C973A + 2, 0x73);

	// These allow slot IDs up to 52 (0x34), so we raise limit to 53 (0x35)
	WriteByte(0x004F1CA9 + 2, 0x34); // EMBLEM slot ID
	WriteByte(0x007FEFBC + 2, 0x33); // EMBLEM slot ID

	WriteByte(0x007FEE93 + 3, 0x34); // iteration limit
	WriteByte(0x0077EDEF + 2, 0x34); // iteration limit
	WriteByte(0x0077E48D + 3, 0x34); // iteration limit
	WriteByte(0x0077EE07 + 2, 0x34); // iteration limit

	PatchNop(0x007FEF39, 0x007FEF58 - 0x007FEF39);
	CodeCave((void*)putEquipPosition_Hook, 0x007FEF39, 5);
}

// ===== Hook Attach =====
void AttachShoulderHooks() {
	SetHook(true, (void**)&OriginalGetItemCategoryName, GetItemCategoryName_Hook);
	SetHook(true, (void**)&OriginalCUIEquipGetBodyPartFromPoint, CUIEquip_GetBodyPartFromPoint_Hook);
	SetHook(true, (void**)&OriginalGetBodyPartFromItem, GetBodyPartFromItem_Hook);
	SetHook(true, (void**)&OriginalIsCorrectBodyPart, IsCorrectBodyPart_Hook);
}
/////////////////////////////////////////////////////////////////////////////////

/// SHADOW PARTNER FIX 1s
const DWORD dwShadowPartnerFixHook = 0x0079023C;
const DWORD dwShadowPartnerFixRetn = 0x0079024B;


__declspec(naked) void ccShadowPartnerFix() {
	__asm {
		push edx
		push ebp
		mov ebp, edx
		...
		pop edx
		jmp dword ptr[dwShadowPartnerFixRetn]
	}
}

void HookShadowPartnerFix() {
	CodeCave((void*)&ccShadowPartnerFix, dwShadowPartnerFixHook, 5);
}


// ===== POS Hooks =====
inline constexpr DWORD dwPos1Retn = 0x0086437D;
inline constexpr DWORD dwPos2Retn = 0x0086449A;
inline constexpr DWORD dwPos3Retn = 0x008645B9;

__declspec(naked) void ccPos1() {
	__asm {
		push 133
		sub eax, edi
		push eax
		jmp dword ptr[dwPos1Retn]
	}
}

__declspec(naked) void ccPos2() {
	__asm {
		push 165
		sub eax, edi
		push eax
		jmp dword ptr[dwPos2Retn]
	}
}

__declspec(naked) void ccPos3() {
	__asm {
		push 186
		sub eax, edi
		push eax
		jmp dword ptr[dwPos3Retn]
	}
}

inline void HookPosHooks() {
	WriteByte(0x0086425B + 1, 82);          // 5
	CodeCave(ccPos1, 0x00864378, 5);        // 56
	CodeCave(ccPos2, 0x00864495, 5);        // 89
	CodeCave(ccPos3, 0x008645B4, 5);        // 108
	WriteValue(0x008646B8 + 1, 221);          // 146
}

//////////////////////////////////////////////////////////

//BATTLESHIP WITH SKILLS////
...
void __declspec(naked) shipSkills() {
	...
	if (nSkillID == 1050 || nSkillID == 10001050 || nSkillID == 20001050 ||
		nSkillID == 1054 || nSkillID == 10001054 || nSkillID == 20001054 ||

		nSkillID == 1001 || nSkillID == 1002 || nSkillID == 1005 || nSkillID == 1001003 ||
		nSkillID == 1101007 || nSkillID == 1101006 || nSkillID == 1111007 || nSkillID == 1111002 ||
		nSkillID == 1121010 || nSkillID == 1121011 || nSkillID == 1121000 || nSkillID == 1121002 ||
		nSkillID == 1201007 || nSkillID == 1201006 || nSkillID == 1211004 || nSkillID == 1211006 ||
		nSkillID == 1211008 || nSkillID == 1211009 || nSkillID == 1211003 || nSkillID == 1211005 ||
		nSkillID == 1211007 || nSkillID == 1221004 || nSkillID == 1221012 || nSkillID == 1221000 ||
		nSkillID == 1221002 || nSkillID == 1221003 || nSkillID == 1301007 || nSkillID == 1301006 ||
		nSkillID == 1301005 || nSkillID == 1301004 || nSkillID == 1311008 || nSkillID == 1311007 ||
		nSkillID == 1320008 || nSkillID == 1321007 || nSkillID == 1321010 || nSkillID == 1320009 ||
		nSkillID == 1321000 || nSkillID == 1321002 || nSkillID == 2001002 || nSkillID == 2001003 ||
		nSkillID == 2101001 || nSkillID == 2101003 || nSkillID == 2111005 || nSkillID == 2121008 ||
		nSkillID == 2121004 || nSkillID == 2121002 || nSkillID == 2121000 || nSkillID == 2201001 ||
		nSkillID == 2201003 || nSkillID == 2211005 || nSkillID == 2221008 || nSkillID == 2221004 ||
		nSkillID == 2221002 || nSkillID == 2221000 || nSkillID == 2301003 || nSkillID == 2301004 ||
		nSkillID == 2311001 || nSkillID == 2311003 || nSkillID == 2311002 || nSkillID == 2321009 ||
		nSkillID == 2321005 || nSkillID == 2321004 || nSkillID == 2321002 || nSkillID == 2321000 ||
		nSkillID == 3001003 || nSkillID == 3101004 || nSkillID == 3111002 || nSkillID == 3121008 ||
		nSkillID == 3121009 || nSkillID == 3121000 || nSkillID == 3121002 || nSkillID == 3201004 ||
		nSkillID == 3211002 || nSkillID == 3221006 || nSkillID == 3221008 || nSkillID == 3221000 ||
		nSkillID == 3221002 || nSkillID == 4001003 || nSkillID == 4101004 || nSkillID == 4111001 ||
		nSkillID == 4111002 || nSkillID == 4121009 || nSkillID == 4121000 || nSkillID == 4121006 ||
		nSkillID == 4201003 || nSkillID == 4211005 || nSkillID == 4211003 || nSkillID == 4221008 ||
		nSkillID == 4221000 || nSkillID == 4221004 || nSkillID == 5001005 || nSkillID == 10000012 ||
		nSkillID == 10001005 || nSkillID == 10001002 || nSkillID == 10001001 || nSkillID == 11111001 ||
		nSkillID == 11101002 || nSkillID == 11001001 || nSkillID == 11101003 || nSkillID == 11001004 ||
		nSkillID == 11111007 || nSkillID == 11121204 || nSkillID == 12101005 || nSkillID == 12001004 ||
		nSkillID == 12111004 || nSkillID == 12001002 || nSkillID == 12001001 || nSkillID == 12101000 ||
		nSkillID == 12101001 || nSkillID == 13111005 || nSkillID == 13101002 || nSkillID == 13001002 ||
		nSkillID == 13111004 || nSkillID == 13101003 || nSkillID == 13001004 || nSkillID == 13101006 ||
		nSkillID == 14001005 || nSkillID == 14001003 || nSkillID == 14101003 || nSkillID == 14111000 ||
		nSkillID == 15001003 || nSkillID == 15100004 || nSkillID == 15111001 || nSkillID == 15101002 ||
		nSkillID == 15001004 || nSkillID == 15111006 || nSkillID == 15101006 || nSkillID == 15111005 ||
		nSkillID == 20000012 || nSkillID == 20001002 || nSkillID == 20001005 || nSkillID == 20001001 ||
		nSkillID == 21121000 || nSkillID == 21121008 || nSkillID == 21100005 || nSkillID == 21111005 ||
		nSkillID == 21101003 || nSkillID == 21111001 || nSkillID == 21120007 || nSkillID == 21000000 ||
		nSkillID == 13121008 || nSkillID == 12111002 || nSkillID == 2211004 || nSkillID == 2111004 ||
		nSkillID == 2311005 || nSkillID == 5221014 || nSkillID == 5221018 || nSkillID == 5121009 ||
		nSkillID == 5121000 || nSkillID == 5221000 || nSkillID == 5221010 || nSkillID == 5121008
		|| nSkillID == 11001001 || nSkillID == 5211006 || nSkillID == 5221016)
...
void InitCorsairMods() {
	WriteByte(0x00967235, 0x75);
	WriteByte(0x00967191 + 1, 0x84);
	SetHook(true, reinterpret_cast<void**>(&dwShipSkills), shipSkills);
}

//// HYPER TELE ROCK STUFF///////////////////////////////////////////////////////
struct WorldMapSubItem
{
	// 0x9EE8D6, total size 68 bytes
	int x;
	int y;
	int type;
	char* streetName;
...
{
	char pad[0x5B4];
	ZArray<WorldMapSubItem> subItems; // Located at +5B4
};

typedef int(__cdecl* CUtilDlg__YesNo_t)(void* sMsg, bool bCenter, bool bFixedW);
static auto CUtilDlg__YesNo = reinterpret_cast<CUtilDlg__YesNo_t>(0x00992DFD);

typedef void(__fastcall* ZXString_char___Assign_t)(void* _this, void* edx, const char* s, int n);
static auto ZXString_char___Assign = reinterpret_cast<ZXString_char___Assign_t>(0x00414617);



auto pWorldMapOnMouseButton = (void(__thiscall*)(void* _this, unsigned int a2, unsigned int a3, int a4, int a5))(0x009EE6E5);
void __fastcall Hook_WorldMapOnMouseButton(void* _this, void* _EDX, unsigned int a2, unsigned int a3, int32_t x, int32_t y)
{

	WorldMapPanel* __this = static_cast<WorldMapPanel*>(_this);
	const size_t count = __this->subItems.GetCount();
	int32_t fix_x = x - 10;
	int32_t fix_y = y - 35;
	boolean sendpacket = false;
	boolean canTP = false;
	unsigned char* ucharPointer = nullptr;

	for (size_t i = 0; i < count && a2 == 514; i++)
	{
		const auto subItem = &__this->subItems[i];
		int distance = ((fix_x - subItem->x) * (fix_x - subItem->x)) + ((fix_y - subItem->y) * (fix_y - subItem->y));

		if (distance <= 17) {
			if (subItem->type == 2) { break; }
			auto mapName = subItem->mapName;
			void* msg = nullptr;
			...
			if (mapName != nullptr) {
				...
				auto streetName = subItem->streetName;
			...
			if (canTP) {
				...
				if (CUtilDlg__YesNo(msg, true, false) == 6) {
					...
					buffer[0] = 0xF1;
					buffer[1] = 0x00;
					buffer[2] = (subItem->mapData[0] >> 0) & 0xFF;
					buffer[3] = (subItem->mapData[0] >> 8) & 0xFF;
					buffer[4] = (subItem->mapData[0] >> 16) & 0xFF;
					buffer[5] = (subItem->mapData[0] >> 24) & 0xFF;
					ucharPointer = buffer;
					SendPacket(ucharPointer, 6);
					sendpacket = true;
...
void HookWorldMapOnMouseButton() {
	SetHook(true, reinterpret_cast<void**>(&pWorldMapOnMouseButton), Hook_WorldMapOnMouseButton);
}
// FINISH HYPER TELE ROCK STUFF /////////////////////////


void YourEdits() {
	HookPosHooks();
	ApplyCooldownUIFix();
	HookShadowPartnerFix();
	ApplyShoulderPatches();

	AttachShoulderHooks();

	PatchNop(0x007C6EF7, 6);

	HookWorldMapOnMouseButton();

	//Custom skills while battleship wowowowoow
	InitCorsairMods();
	...
	//DK RUSH
	WriteValue(0x00952E1F + 3, 1321003);
	WriteValue(0x00952114 + 1, 1321003);
	WriteValue(0x00950B69 + 1, 1321003);
	...
	//Speed Cap Removal
	WriteValue(0x00780746, 600);
	WriteValue(0x008c4287, 600);
	WriteValue(0x0094D91F, 600);

	//Ladder/Climb Speed
	WriteValue(0x009CC6F9 + 2, 0x00C1CF80); //switch addy
	WriteDouble(0x00C1CF80, 10.0); //Addy speed control
	...
	WriteValue(0x00960581 + 3, m_nGameWidth - 100);
	WriteValue(0x00960839 + 1, m_nGameWidth - 100);
	WriteValue(0x00960C67 + 1, m_nGameWidth - 100);
	WriteValue(0x00960DED + 1, m_nGameWidth - 100);
