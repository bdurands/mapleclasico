    ...
    if (status == 1) {
        ...
        if (cm.getMeso() >= price) {
            if (selection >= 1 && selection <= 3) {
                var chunk = Math.ceil(hair.length / 3);
                var selectedHair = hair.slice((selection - 1) * chunk, selection * chunk);
                for each(var i in selectedHair) {
                    pushIfItemExists(hairnew, i);
                }
                cm.sendStyle("Pick one?", hairnew);
            } else if (selection >= 5 && selection <= 7) {
                var chunk = Math.ceil(fhair.length / 3);
                var selectedHair = fhair.slice((selection - 5) * chunk, (selection - 4) * chunk);
                for each(var i in selectedHair) {
                    pushIfItemExists(hairnew, i);
                }
                cm.sendStyle("Pick one?", hairnew);
        ...
        if ((beauty >= 2 && beauty <= 4) || (beauty >= 6 && beauty <= 8)) { 
