...
public class StorageInventory {
    ...
    // int, NOT byte: the ore/scroll/chair/mount bags hold up to 200 items. Casting the item count to a
    // byte overflowed to a NEGATIVE slotLimit at >127 items, which made getNextFreeSlot()/isFull() reject
    // every addItem in the constructor -> mergeItems() ran on an empty map and sortItems() returned an
    // empty list, WIPING a full bag on the "organize" button. (Native storage never exceeds 127, so this
    // latent bug only surfaced once the 200-slot bags started reusing this class.)
